...
export function FlashDeals() {
  const [left, setLeft] = useState(7 * 3600 + 42 * 60 + 15);

  useEffect(() => {
    const t = setInterval(() => setLeft((p) => (p <= 0 ? 8 * 3600 : p - 1)), 1000);
  ...
  return (
    <section className="mt-3 bg-navy py-4">
      <div className="flex items-center justify-between px-4">
        ...
        <button className="flex items-center text-[12px] font-semibold text-orange-ring active:opacity-70">
      ...
      <div className="mt-3 flex gap-3 overflow-x-auto no-scrollbar px-4">
        {flashDeals.map((p) => (
          <ProductCard key={p.id} product={p} width="w-[144px] shrink-0" />
        ))}
      </div>
    </section>
  );
